...
public class ConexionVerTempEntradas {
    Connection conex;
    ...
    public void documento()
    {
            ...
            try
      {
        ...
        consulta= conex.prepareStatement("select fecha_documentos, cantidad_articulos, conceptos, valor_operacion, proveedores from temporal_doc_entrada where doc_entrada=? and seccion=?");
        consulta.setString(1, documento);
        consulta.setInt(2, seccion);
        ejecutar=consulta.executeQuery();
        while( ejecutar.next() )
        {
                     fecha=(ejecutar.getDate("fecha_documentos"));
                     ...
                     cod_proveedor=ejecutar.getString("proveedores");
