...
public class Prueba {
   ...
   /*
    ...
    HSSFRow filas= hoja.createRow(0);//fila
    HSSFCell celda=filas.createCell((short)0);//celda
    HSSFRichTextString texto=new HSSFRichTextString("HOLA MUNDOO");//contenido a incluir en celda
   
    public void ejecutar() throws IOException
    {
         celda.setCellValue(texto);
        try {
             try (FileOutputStream archiv = new FileOutputStream("prueba.xls")) {
                 libro.write(archiv);
             }
...
for(int i=0; i<20; i++)
{
      ...
      HSSFCell celdaB = fila2.createCell(j);
        ...
        try {
            ...
            File archivoExcel = new File ("holamundo.xls");
